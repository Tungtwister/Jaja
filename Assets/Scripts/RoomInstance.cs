...
public class RoomInstance : MonoBehaviour {
	...
	[SerializeField]
	GameObject doorU, doorD, doorL, doorR, doorWall, enemy;
	[SerializeField]
	...
	void PlaceDoor(Vector3 spawnPos, bool door, GameObject doorSpawn){
		...
		if (door){
			Instantiate(doorSpawn, spawnPos, doorSpawn.transform.rotation).transform.parent = transform;
		}else{
	...
	void GenerateTile(int x, int y){
		...
		//find the color to macth the pixel
